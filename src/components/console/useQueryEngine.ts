...
import { getInclusiveMonths, getAge } from './dateUtils';
...
export function useQueryEngine() {
  ...
  const runSqlQuery = useCallback(async (sqlQuery: string) => {
  ...
  const executeQuery = useCallback((query: string, mode: QueryMode) => {
    if (mode === 'sql') {
      runSqlQuery(query);
    } else {
